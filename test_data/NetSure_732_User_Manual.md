# Chunk 1
NetSure 732                       NetSure 732 Subrack Power System User Manual
...
# Chunk 5
# Revision date December 22, 2019
...
Copyright © 2014 by Vertiv., Ltd. All rights reserved. The contents in this document are subject to change without notice.
...
```
[Image of a square unit with labeled parts:
Square unit
Cable outlet area
Cable-bundling plate
Cable outlet area
Signal cable outlet holes]
```
...
```
[Image of a latex unit with labeled parts:
Latex unit
Cable outlet area]
```
...
```mermaid
graph TD
    A[W2453X1 VER A01 2009.12.25] --> B[FE]
    A --> C[RIc]
    A --> D[RLY1]
    A --> E[RLY2]
    A --> F[RLY3]
...
```mermaid
graph TD
    A[M225S Controller] --> B[ESC Button]
    A --> C[ENT Button]
    A --> D[RS232 communication serial port]
    A --> E[Dry contact output]
    D --> F[Rx Tx]
```
...
|                                | DC output over-voltage recovery point            | Default: -57.5 ± 0.2Vdc, 0.5Vdc lower than the over-voltage alarm point                                                         |
| DC output alarm and protection | DC output under-voltage alarm point              | Default: -45.0 ± 0.2Vdc, configurable through controller                                                                        |
...
graph TD
    ...
    style A width:200px,height:200px
    style B width:200px,height:20px
    style C width:20px,height:200px
```
...
graph TD
    subgraph "Top View"
        ...
        7-J7-8 --> COM
        2-N-2 --> 2-L
        DO3 & DO4 & DO5 & DO6 --> 31-J1-L/31-J2-L/31-J3-L/31-J4-L/3-2
        7-J6-2 & 7-J6-1 --> 2-N-1
        2-N-1 --> 1-2
        QFD9 & QFD3 & QFD1 & QFD12 & QFD8 & QFD2 --> 8-PL-2
        7-J7-11 --> 7-J6-10
        7-J6-9 --> QFB1
        18-J2-1 & 18-J2-2 --> QFB2
        9-BUS+-2 --> 7-J6-7
        7-J6-8 --> W07
        W07 --> 5-4
    end

    subgraph "Front View of Subrack"
        J7 --> W06
        QFA --> W01
        J4 --> W80
        M2433X2 --> J2
        ...
        W80 --> CAN+
        CAN- --> J1
        31-J1-N/31-J2-N/32-J3-N/32-J4-N --> J5
        PE --> J3
        DCSPD --> U3
        QFA2 --> W01
        AC_input --> Single_phase_rectifier
        31W2493ZX1 --> DC+/DC-
        W80 --> J42
    end

    subgraph "Right View"
        Pull_out_controller --> J2
        interface_board --> W80
        M225S1X1 --> User_module
    end

    subgraph "Rear View"
        This_contains_KM1_KM2 --> diode
        To_PL --> 15-J78-PL
        M225S --> MFU
        motherboard --> DC_monitor_socket
        M34C3C1
    end
```
...
graph TD
    ...
    subgraph "Front top view"
        FTV[Front top view]
    end

    subgraph "Controller"
        C1[Controller]
        C2[Controller interface board]
    end

    subgraph "Power Supply"
        PS1[AC input]
        PS2[Single-phase rectifier]
    end

    subgraph "Connections"
        CON1[X5-1 to X6-1]
        CON2[To the positive busbar]
        CON3[To the negative busbar]
        CON4[To the temperature sensor cable]
    end
...
```mermaid
graph TD
    AC[AC Distribution Unit] --> SPD1
    SPD1 --> QFA1
    QFA1 --> QFA2
    QFA2 --> J1[J1 M225S1X1]
    J1 --> Controller[M225S Controller]
    ...
    KMD1 --> Battery1[Battery 1]
    KMD2 --> Battery2[Battery 2]
    
    Controller --> |BLVD_CTRL+/- DI_DOOR+/- DI_SPD+/-| PD3[PD3]
    Controller --> |LOAD_FUSE BAT_FUSE1/2 BAT1_SH+/-| PD2[PD2]
    Controller --> |CAN+/- OC1+/- OC2+/-| PD1[PD1]
...
The schematic diagram shows the electrical connections and components of a power distribution system, likely for a telecommunications or industrial application. Key components include:

1. AC Distribution Unit
2. Surge Protection Device (SPD1)
3. Circuit Breakers (QFA1, QFA2)
4. M225S Controller
5. DC-DC Converters (U1, U2, U3, U4)
6. DC Busbar
7. Multiple DC Circuit Breakers (QFD1, QFD2, QFD3, etc.)
8. Load Disconnects (1D1, 1D2, 1D3, etc.)
9. Low Voltage Disconnect (LLVD) and Battery Low Voltage Disconnect (BLVD)
10. Battery Contactors (KMD1, KMD2)
11. Batteries
12. Various connectors and interfaces (J1, J2, J3, etc.)

The system appears to convert AC power to DC, distribute it to various loads, and manage battery backup. The M225S controller likely monitors and controls the entire system, including battery management and load shedding.
...
```mermaid
graph TD
    AC_Distribution_Unit --> QFA1
    QFA1 --> Main_Distribution_Unit
    Main_Distribution_Unit --> QFA2
    QFA2 --> AC_output
    AC_output --> W34C3C1
    W34C3C1 --> KMD2
    KMD2 --> Terminal
    ...
    Main_Distribution_Unit --> Controller_M221S
    Controller_M221S --> DO1 & DO2 & DO3 & DO4
    
    Controller_M221S --> LLVD_Contactor
    LLVD_Contactor --> QFD9
    
    Controller_M221S --> W2453X1
    W2453X1 --> J72 & J8 & J6 & J4 & J3
    
    Main_Distribution_Unit --> KMD1
    KMD1 --> QFD1 & QFD2 & QFD3 & QFD8
    
    QFD1 --> X6-1
    QFD2 --> X5-1
    
    Main_Distribution_Unit --> DC_SPD
    DC_SPD --> Positive_Busbar
    
    Positive_Busbar --> QFB1 & QFB2
    QFB1 --> Battery1
    QFB2 --> Battery2
    
    Main_Distribution_Unit --> H1 & H2 & H3 & H4
