...
def disable_text_wrapping(cell):
    """Disable text wrapping in a Word table cell."""
    tc = cell._tc
    tcPr = tc.get_or_add_tcPr()
    noWrap = OxmlElement('w:noWrap')
    noWrap.set('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val', "true")
    tcPr.append(noWrap)
    ...
    if table_json["headers"] and table_json["data"]:
        ...
        for i, header in enumerate(headers_first_3):
            if i < len(header_cells):
                ...
                disable_text_wrapping(header_cells[i])  # ✅ giữ nguyên text, không xuống dòng
                for paragraph in header_cells[i].paragraphs:
                    for run in paragraph.runs:
                        run.font.bold = True
        set_repeat_table_header(table.rows[0])  # ✅ header lặp lại
            ...
            if row_idx + 1 < len(table.rows):
                ...
                for col_idx, cell_data in enumerate(row_data_first_3):
                    if col_idx < len(row_cells):
                        ...
                        disable_text_wrapping(row_cells[col_idx])  # ✅ giữ text một dòng
